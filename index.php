...
if(isset($_GET['profile']))
{
	...
	if ($result = mysqli_query($conn, $query))
	{
		while ($db = mysqli_fetch_object($result))
		{
			...
			{
				echo '
				<h2>'.$db->name.' - '.readTKNumber($db->tkid).'</h2>
			...
			echo '
			<tr>
				<td><a href="index.php?event='.$db->troopid.'">'.$db->eventName.'</a></td>	<td>'.getCostume($db->costume).'</td>	<td>'.ifEmpty($db->attended_costume).'</td>
			</tr>';
...
if(isset($_GET['action']) && $_GET['action'] == "search")
{
	...
	if ($result = mysqli_query($conn, $query))
	{
		while ($db = mysqli_fetch_object($result))
		{
			...
			echo '
			<tr>
				<td><a href="index.php?event='.$db->troopid.'">'.$db->eventName.'</a></td>	<td><a href="index.php?profile='.$db->trooperid.'">'.getTKNumber($db->trooperid).'</a></td>	<td>'.getCostume($db->costume).'</td>	<td>'.ifEmpty($db->attended_costume).'</td>	<td>'.didAttend($db->attend).'</td>
			</tr>';
...
if(isset($_GET['action']) && $_GET['action'] == "login")
{
	...
	else
	{
		...
		<p><a href="index.php?action=forgotpassword">[Forgot Your Password]</a><p>';
	}
}
...
if(isset($_POST['submitConfirmList']))
{
	...
	if(!empty($list)) 
	{
		...
		{
			// Query the database
			$conn->query("UPDATE event_sign_up SET attend = '1' WHERE trooperid = '".$_SESSION['id']."' AND troopid = '".cleanInput($list[$i])."'") or die($conn->error);
		}
	}
	...
	if(!empty($list2)) 
	{
		...
		for($i = 0; $i < $n; $i++)
		{
			...
			// Query the event sign up database
			$conn->query("UPDATE event_sign_up SET attend = '1' WHERE trooperid = '".$_SESSION['id']."' AND troopid = '".cleanInput($arraySplit[1])."'") or die($conn->error);

			// Query the shift database
			$conn->query("UPDATE shift_trooper SET attend = CONCAT(attend, ',".$arraySplit[0]."') WHERE trooperid = '".$_SESSION['id']."' AND troopid = '".cleanInput($arraySplit[1])."' AND id = '".$arraySplit[0]."'") or die($conn->error);
		}
	}
}
...
if(isset($_POST['submitConfirmListDelete']))
{
	...
	if(!empty($list2)) 
	{
		...
		for($i = 0; $i < $n; $i++)
		{
			...
			// Query the event sign up database
			$conn->query("UPDATE event_sign_up SET attend = '2' WHERE trooperid = '".$_SESSION['id']."' AND troopid = '".cleanInput($arraySplit[1])."'") or die($conn->error);

			// Query the shift database
			$conn->query("UPDATE shift_trooper SET attend = '2' WHERE trooperid = '".$_SESSION['id']."' AND troopid = '".cleanInput($arraySplit[1])."' AND id = '".$arraySplit[0]."'") or die($conn->error);
		}
	}
}
		...
		while ($db = mysqli_fetch_object($result))
		{
			...
			if ($result2 = mysqli_query($conn, $query2))
			{
				while ($db2 = mysqli_fetch_object($result2))
				{
					...
					echo '
					<tr>
						<td><a href="index.php?profile='.$db2->trooperId.'">'.$db2->name.'</a></td>	<td>'.readTKNumber($db2->tkid).'</td>	<td>'.getCostume($db2->costume).'</td>	<td>'.$db2->costume_backup.'</td>	<td>'.getStatus($db2->status).'</td>';
			...
			if($i == 0)
			{
				echo '<b>No troopers have signed up for this event!</b>';
			}
			else
			{
				echo '</table>
				...
				else
				{
					...
					echo '
						...
						echo '
							...
							<input type="text" name="backupcostume" id="backupcostume" />';
...
else
{
	...
	if(!isset($_GET['action']))
	{
		...
		/*echo '
		<a href="index.php">[ALL]</a> <a href="index.php?squad=1">[Everglades Squad]</a> <a href="index.php?squad=2">[Makaze Squad]</a> <a href="index.php?squad=3">[Parjai Squad]</a> <a href="index.php?squad=4">[Squad 7]</a> <a href="index.php?squad=5">[Tampa Bay Squad]</a>
		...
		<a href="index.php"><img src="images/garrison_emblem.png" alt="Florida Garrison Troops" /></a> <a href="index.php?squad=1"><img src="images/everglades_emblem.png" alt="Everglades Squad Troops" /></a> <a href="index.php?squad=2"><img src="images/makaze_emblem.png" alt="Makaze Squad Troops" /></a> <a href="index.php?squad=3"><img src="images/parjai_emblem.png" alt="Parjai Squad Troops" /></a> <a href="index.php?squad=4"><img src="images/squad7_emblem.png" alt="Squad 7 Troops" /></a> <a href="index.php?squad=5"><img src="images/tampabay_emblem.png" alt="Tampa Bay Squad Troops" /></a>
		...
		if ($result = mysqli_query($conn, $query))
		{
			while ($db = mysqli_fetch_object($result))
			{
				...
				if(!isset($_GET['squad']))
				{
					echo '<a href="index.php?event=' . $db->id . '">[' .date('M d, Y', strtotime($db->dateStart)). ' - '.date('M d, Y', strtotime($db->dateEnd)).']'.' ' . $db->name . '</a>';

					if($getNumOfTroopers->num_rows <= 1)
					{
						echo ' <span style="color:red;"><b>NOT ENOUGH TROOPERS FOR THIS EVENT!</b></span>';
					}
				}
				else
				{
			...
			if ($result = mysqli_query($conn, $query))
			{
				...
				while ($db = mysqli_fetch_object($result))
				{
					if($i == 0)
					{
						...
						<h2>Confirm Troops</h2>
						<form action="index.php" method="POST" name="confirmListForm" id="confirmListForm">';
					}

					// Load shifts if applicable

					// Query database for shift info
					$query3 = "SELECT shift_trooper.id, shift_trooper.shift, shift_trooper.troopid, shift_trooper.trooperid FROM shift_trooper WHERE shift_trooper.trooperid = '".$_SESSION['id']."' AND shift_trooper.troopid = '".$db->eventId."'";
					...
					if ($result3 = mysqli_query($conn, $query3))
					{
						while ($db3 = mysqli_fetch_object($result3))
						{
							...
							for($n = 0; $n <= count($shiftString) - 1; $n += 2)
							{
								...
								$data .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $days[$shiftString[$n + 1]] . '<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="confirmShift[]" id="confirmShift_'.$shift[0].'" value="'.$db3->id.','.$db->eventId.'" /> ' . $readTime1 . ' - ' . $readTime2 . '<br /><br />';
							}
