...
if(isset($_GET['do']) && $_GET['do'] == "editevent" && loggedIn() && isAdmin())
{
	...
	if(isset($_POST['submitEditRoster']))
	{
		...
		// Query database for shift info
		$query3 = "SELECT shift_trooper.shift, shift_trooper.troopid, shift_trooper.trooperid, events.dateStart, events.dateEnd FROM shift_trooper LEFT JOIN events ON events.id = shift_trooper.troopid WHERE shift_trooper.trooperid = '".cleanInput($_POST['trooperSelectEdit'])."' AND shift_trooper.troopid = '".cleanInput($_POST['eventId'])."'";
	...
	if(isset($_POST['submitRoster']))
	{
		...
		if ($result = mysqli_query($conn, $query))
		{
			while ($db = mysqli_fetch_object($result))
			{
				...
				if($i == 0)
				{
					...
					<form action="process.php?do=editevent" method="POST" name="troopRosterForm" id="troopRosterForm">
						...
						<table border="1" name="rosterTable" id="rosterTable">
							<tr>
								<th>Selection</th>	<th>Trooper TKID</td>	<th>Trooper Costume</th>	<th>Trooper Backup Costume</th>	<th>Trooper Status</th>	<th>Trooper Comment</th>	<th>Trooper Attended</th>	<th>Attended With</th>	<th>Dates Attending</th>';
				}
				...
				<tr id="roster_'.$db->trooperid.'" name="roster_'.$db->trooperid.'">
					...
					<td>
						<div name="backup1'.$db->trooperid.'" id="backup1'.$db->trooperid.'">'.$db->costume_backup.'</div>
						<div name="backup2'.$db->trooperid.'" id="backup2'.$db->trooperid.'" style="display:none;"><input type="text" name="costumeVal'.$db->trooperid.'" id="costumeVal'.$db->trooperid.'" /></div>
					</td>
					...
					<td>
						<div name="attendcostume1'.$db->trooperid.'" id="attendcostume1'.$db->trooperid.'">'.$db->attended_costume .'</div>
						<div name="attendcostume2'.$db->trooperid.'" id="attendcostume2'.$db->trooperid.'" style="display:none;"><input type="text" name="attendcostumeVal'.$db->trooperid.'" id="attendcostumeVal'.$db->trooperid.'" /></div>
					</td>
					...
					foreach ($days as $key => $value)
					{
						...
						$query3 = "SELECT shifts.id, shifts.starttime, shifts.endtime, shifts.troopid AS shiftTroop, shift_trooper.troopid, shift_trooper.shift, shift_trooper.trooperid FROM shifts LEFT JOIN shift_trooper ON shifts.troopid = shift_trooper.troopid WHERE shifts.troopid = '".$db->troopid."'";
		...
		else
		{
			...
			echo '
				...
				<input type="text" name="costumebackup" id="costumebackup" />
