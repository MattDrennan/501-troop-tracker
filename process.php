...
if(isset($_GET['do']) && isset($_GET['event']) && $_GET['do'] == "load-add-friend" && loggedIn())
{
	...
	$statement = $conn->prepare("SELECT * FROM costumes WHERE club NOT EXISTS (".implode(",", $dualCostume).") ORDER BY FIELD(costume, ".$mainCostumes."".mainCostumesBuild($_SESSION['id']).") DESC, costume");
	$statement->execute();
	...
	// Display costumes
	$statement = $conn->prepare("SELECT * FROM costumes WHERE club NOT EXISTS (".implode(",", $dualCostume).") AND " . costume_restrict_query(0, false, false) . " ORDER BY FIELD(costume, ".$mainCostumes."".mainCostumesBuild($_SESSION['id']).") DESC, costume");
	$statement->execute();
